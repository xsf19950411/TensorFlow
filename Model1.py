...
with tf.name_scope('Layer1'):
	...
	tf.summary.histogram('Biase1', b1)

with tf.name_scope('Layer2'):
	W2=tf.Variable(tf.random_normal([2000, 10], 0, 0.01))
	b2=tf.Variable(tf.zeros([1, 10]))
	Output2=tf.matmul(Output1, W2)+ b2
	tf.summary.histogram('Weight2', W2)
	tf.summary.histogram('Biase2', b2)

with tf.name_scope('Loss'):
	Loss= tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=y_, logits=Output2))
...
with tf.name_scope('Train'):
	...
	#clipped_gradients= tf.clip_by_value(gradients, 0, 1)
	Train_op= opt.apply_gradients(zip(gradients, params))

init= tf.global_variables_initializer()
merged= tf.summary.merge_all()
writer= tf.summary.FileWriter("F:/Files/Program/py/TensorFlow/Model1/Summary", sess.graph)
...
sess.run(init)
for i in range(300):
	sess.run(Train_op, feed_dict={xs: x_data, y_: y_data})
	if i%10 ==0:
