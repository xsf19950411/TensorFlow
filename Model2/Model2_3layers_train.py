...
###########生成训练数据集####################
frequencyStep= 0.005
y_train= np.zeros((batchNum, 1, dataLength, 1))
x_train= np.zeros((batchNum, 1, dataLength, 1))
for i in range(batchNum):
	randFreq= np.random.randint(5)
	y_train[i, 0, :, 0]= np.sin(2*3.14*(0.010+randFreq*frequencyStep)*np.linspace(0, dataLength, dataLength))
...
for i in range(200):
	...
	if i%20 ==0:
		result=sess.run(merged, feed_dict={x: x_train, y: y_train})
		writer.add_summary(result, 200+ i)
#############模型训练--第三层###############
for i in range(1800):
	sess.run(Train_op3, feed_dict={x: x_train, y: y_train})
	if i%20 ==0:
		result=sess.run(merged, feed_dict={x: x_train, y: y_train})
		writer.add_summary(result, 400+ i)

savePath=saver.save(sess, 'F:\Files\Program\py\TensorFlow\Model2\Model2.ckpt')
